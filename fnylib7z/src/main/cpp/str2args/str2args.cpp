...
//                __android_log_print(ANDROID_LOG_VERBOSE,"libfnyLib7z.so","Parse Error! Bad quotes\n");
                ret = false;
                break;
            case '\\':
    ...
    *argc = arg_count;
    ...
    if (in_container) {
        __android_log_print(ANDROID_LOG_WARN,"libfnyLib7z.so","Parse Error! Still in container\n");
        ret = false;
    }
    if (escaped) {
        __android_log_print(ANDROID_LOG_WARN,"libfnyLib7z.so","Parse Error! Unused escape (\\)\n");
        ret = false;
    }
    return ret;
}
