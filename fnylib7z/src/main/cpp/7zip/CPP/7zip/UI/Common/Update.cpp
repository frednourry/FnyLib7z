...
// FNY
FString CArchivePath::GetTempPath2(UString newPath) const {
  if (newPath.Len() > 0) {
    FString path = TempPrefix;
    path += us2fs(newPath);
    return path;
    ...
    else
    {
      ...
      for (unsigned i = 0; i < (1 << 16); i++)
      {
        if (archivePath.Temp)
        {
          ...
          if (options.OutFilename.Len()>0)
            realPath = archivePath.GetTempPath2(options.OutFilename); // FNY
          else
    ...
    bool needSetPath)
{
  ...
  {
    NFind::CFileInfo fi;
    if (!fi.Find(us2fs(arcPath)) && options.IdFileDescriptor <= 0)
    {
      if (renameMode)
    ...
    else
    {
      ...
      op.stream = NULL;
      op.filePath = arcPath;
      op.idFileDescriptor = options.IdFileDescriptor;
