...
static const CSwitchForm kSwitchForms[] =
{
  ...
  { "fny-stdout",  NSwitchType::kString, false, 1 },   // FNY  - (string) Filename for the Stdout (but it's already catch in Console\Main.cpp)
  { "fny-stderr",  NSwitchType::kString, false, 1 },   // FNY  - (string) Filename for the Stderr (but it's already catch in Console\Main.cpp)
...
void CArcCmdLineParser::Parse2(CArcCmdLineOptions &options)
{
  ...
  // FNY
  // -fny-stdout[filename] option
  if (parser[NKey::kStdoutFilename].ThereIs) {
    const UString &postString = parser[NKey::kStdoutFilename].PostStrings[0];
    if (!postString.IsEmpty())
      options.StdoutFilename = us2fs(postString);
  }

  // -fny-outfile[filename] option
  if (parser[NKey::kOutFilename].ThereIs) {
    const UString &postString = parser[NKey::kOutFilename].PostStrings[0];
    if (!postString.IsEmpty())
      options.OutFilename = us2fs(postString);
  }

  // -fny-fdin[file descriptor] option
  if (parser[NKey::kFileDescriptorId].ThereIs) {
    const UString &fdString = parser[NKey::kFileDescriptorId].PostStrings[0];
    UInt64 id;
    if (!ParseComplexSize(fdString, id)) {
      throw CArcCmdLineException("Incorrect file descriptor value:", fdString);
    }
    options.IdFileDescriptor = id;
  }

  // -fny-n[list of item indexes] option
  if (parser[NKey::kItemsToExtract].ThereIs) {
    CIntVector itemsToExtract;
    const UString &numItemsString = parser[NKey::kItemsToExtract].PostStrings[0];
    UInt64 num;

    // Parse numbers separated by a coma
    wchar_t coma = ((wchar_t)',');
    int len = numItemsString.Len();
    UString mid = numItemsString;
    int pos = mid.Find(coma);
    while (pos >0) {
        UString left = mid.Left(pos);
        UString mid2 = mid.Mid(pos+1, len);
        len -= left.Len()+1;

        if (!ParseComplexSize(left, num)) {
            throw CArcCmdLineException("Incorrect -n<list of items to extracts> option !");
        }
        itemsToExtract.Add(num);

        mid = mid2;
        pos = mid.Find(coma);
    }
    // Parse the last number
    if (!ParseComplexSize(mid, num)) {
        throw CArcCmdLineException("Incorrect -n<list of items to extracts> option !");
    }
    itemsToExtract.Add(num);

    options.ItemsToExtract = itemsToExtract;
  }
